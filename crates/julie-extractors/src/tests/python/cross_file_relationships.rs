//! Cross-File Relationship Extraction Tests for Python
//!
...
mod tests {
    ...
    fn extract_full(filename: &str, code: &str) -> ExtractionResults {
        ...
        extract_symbols_and_relationships(&tree, filename, code, "python", &workspace_root)
...
"#;
        ...
        let file_b_code = r#"
from file_a import helper_function
...
"#;
        ...
        let results_a = extract_full("lib/file_a.py", file_a_code);
...
def process():
    calc = Calculator(21)  # Cross-file constructor call
    return calc.double()   # Cross-file method call
"#;

        let results_a = extract_full("lib/calculator.py", file_a_code);
        let results_b = extract_full("lib/processor.py", file_b_code);
...
"#;
        ...
        let (symbols, relationships) = extract_from_file("src/same_file.py", code);
