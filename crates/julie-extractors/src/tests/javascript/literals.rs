...
fn capture(code: &str) -> Vec<Literal> {
    ...
    let tree = parser.parse(code, None).expect("parse JavaScript");
...
fn template_string_arg_decodes_interpolation_holes() {
    // `fetch(`/api/users/${id}/orders`)` — the template substitution `${id}` is
    // decoded to a `{}` placeholder so the resolver sees the static URL shape.
    let code = r#"
function load(id) {
