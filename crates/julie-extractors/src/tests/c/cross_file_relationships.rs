...
"#;
        ...
        let results_a = extract_full("src/utils.c", file_a_code);
        let results_b = extract_full("src/main.c", file_b_code);
        ...
        assert!(
            helper_fn.is_some(),
            "Should extract helper_function from utils.c"
        );

        let main_fn = results_b.symbols.iter().find(|s| s.name == "main_function");
        assert!(
            main_fn.is_some(),
            "Should extract main_function from main.c"
        );
...
"#;
        ...
        let (symbols, relationships) = extract_from_file("src/same_file.c", code);
