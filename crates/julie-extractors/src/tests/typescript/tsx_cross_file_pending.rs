//! Phase 4a.tsx — TSX (TypeScriptExtractor) emits `StructuredPendingRelationship`
//! for cross-file class imports referenced via `new` and JSX elements.
//!
//! Source under test: `fixtures/extraction/tsx/cross_file/source.tsx`.
...
#[test]
fn test_tsx_emits_structured_pending_for_cross_file_class() {
    let source = include_str!("../../../../../fixtures/extraction/tsx/cross_file/source.tsx");
    let workspace_root = Path::new("/tmp/test");
    let result = extract_canonical("source.tsx", source, workspace_root)
        .expect("canonical TSX extraction must succeed");
    ...
    assert_eq!(foo.pending.file_path, "source.tsx");
...
#[test]
fn test_tsx_negative_local_helper_not_emitted_as_pending() {
    let source = include_str!("../../../../../fixtures/extraction/tsx/cross_file/source.tsx");
    let workspace_root = Path::new("/tmp/test");
    let result = extract_canonical("source.tsx", source, workspace_root)
        .expect("canonical TSX extraction must succeed");
