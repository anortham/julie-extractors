...
#[test]
fn powershell_complexity_metrics_emit_file_and_symbol_scopes() {
    ...
    let source = r#"
function Evaluate($widget, $count, $enabled) {
    $total = 0
    if ($enabled) {
    ...
    while ($total -gt 100) { $total = [math]::Floor($total / 2) }
    return $total
}
"#;

    let results = extract("src/Evaluate.ps1", source);
