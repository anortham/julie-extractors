...
pub(super) const SPECS: &[StructuralFactPatternSpec] = &[
    ...
    StructuralFactPatternSpec {
        ...
        description: "One clause of a PHP `use` import declaration.",
        metadata_keys: &[
            K_PATTERN_VERSION,
            K_QUERY_FAMILY,
            key(
            ...
            key(
                ...
                STR,
                OPT,
                "`function` or `const` for `use function` / `use const`; absent for a class import.",
            ),
        ],
    },
    StructuralFactPatternSpec {
        ...
        languages: &["php"],
        query_family: "traits",
        description: "One trait named by a PHP `use` mixin inside a class.",
        metadata_keys: &[
            K_PATTERN_VERSION,
            K_QUERY_FAMILY,
            key(
    ...
    StructuralFactPatternSpec {
        ...
        languages: &["php"],
        query_family: "imports",
        description: "A PHP `include`, `include_once`, `require`, or `require_once` expression.",
        metadata_keys: &[
            K_PATTERN_VERSION,
            K_QUERY_FAMILY,
            key(
            ...
            key(
                ...
                STR,
                OPT,
                "The static path: a string literal, or the literal part after `path_base`.",
            ),
            key(
                "path_base",
                STR,
                OPT,
                "`__DIR__` or `dirname(__FILE__)` when the path is joined to the file directory.",
            ),
        ],
    },
    StructuralFactPatternSpec {
