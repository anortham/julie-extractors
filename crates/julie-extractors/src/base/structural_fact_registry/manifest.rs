...
pub(super) const SPECS: &[StructuralFactPatternSpec] = &[
    ...
    StructuralFactPatternSpec {
        ...
        languages: &["toml", "xml", "swift"],
        query_family: "dependencies",
        description: "A package dependency declared in a Cargo.toml, pyproject.toml, MSBuild, NuGet, Maven, or SwiftPM manifest.",
        metadata_keys: &[
            K_PATTERN_VERSION,
            K_QUERY_FAMILY,
            key(
                ...
                STR,
                ALWAYS,
                "Package ecosystem (\"cargo\", \"pypi\", \"nuget\", \"maven\", or \"swiftpm\").",
            ),
            key(
                "name",
                STR,
                ALWAYS,
                "Dependency name: the Cargo key, the PEP 503-normalized distribution name, the NuGet package id, the Maven `groupId:artifactId`, or the SwiftPM package identity (the `name:` argument, else the last URL or path component without `.git`).",
            ),
            key(
                "group",
                STR,
                ALWAYS,
                "Dependency group: Cargo `dependencies`/`dev-dependencies`/`build-dependencies`/`workspace`; Python `runtime`, `optional:<extra>`, `group:<name>`, `build-system`, `poetry:<group>`; NuGet `PackageReference`/`PackageVersion`/`GlobalPackageReference`/`dependency`; Maven scope, `managed`, `plugin`, or `managed-plugin`; SwiftPM `dependencies`.",
            ),
