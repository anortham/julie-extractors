...
pub(super) const SPECS: &[StructuralFactPatternSpec] = &[
    ...
    StructuralFactPatternSpec {
        pattern_id: "rocket.route.v1",
        languages: &["rust"],
        query_family: "framework",
        description: "A static Rocket attribute-macro route (`#[get(\"/x\")]` / `#[route(GET, uri = \"/x\")]`) on a handler fn.",
        metadata_keys: &[
            K_PATTERN_VERSION,
            K_QUERY_FAMILY,
            K_FRAMEWORK,
            ...
            key(
                ...
                STR,
                ALWAYS,
                "Raw static route path from the attribute macro.",
            ),
            key(
                "normalized_route_template",
                STR,
                ALWAYS,
                "Cross-family join key; Rocket `<id>` / `<path..>` captures normalize to `:id` / `:path` segments.",
            ),
            key(
                "dynamic_segments",
                ARR,
            ...
            key(
                ...
                STR,
                ALWAYS,
                "Uppercase HTTP method from the macro name (`#[get]`→GET) or the first `#[route]` argument.",
            ),
            key(
                "verb_source",
                STR,
                ALWAYS,
                "How the verb was attested (\"attested\"; route macros always name the verb).",
            ),
        ],
    },
    StructuralFactPatternSpec {
        pattern_id: "rocket.mount.v1",
        languages: &["rust"],
        query_family: "framework",
        description: "A static Rocket `.mount(\"/lit\", routes![a, b])` registration, the prefix and handler names recorded at the mount site.",
        metadata_keys: &[
            K_PATTERN_VERSION,
            K_QUERY_FAMILY,
            K_FRAMEWORK,
            key("mount_path", STR, ALWAYS, "Raw static mount path literal."),
            key(
                "normalized_mount_path",
                STR,
                ALWAYS,
                "Normalized mount path (Rocket angle captures as `:param`).",
            ),
            key(
                "mount_target",
                STR,
                ALWAYS,
                "Source text of the `routes![..]` argument.",
            ),
            key(
                "handler_names",
                ARR,
                ALWAYS,
                "Handler names the `routes!` list registers (last path segment of each entry); the handlers' own route facts may live in other files.",
            ),
        ],
    },
];
