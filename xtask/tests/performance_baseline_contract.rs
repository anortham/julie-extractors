...
fn writer_current_schema_execution_writes_every_current_child_domain() {
    ...
    assert_eq!(summary.rows_written.complexity_metrics, 8);
...
fn writer_current_schema_summary_serializes_stable_fields() {
    ...
    assert_eq!(value["rows_written"]["complexity_metrics"], 3);
...
fn baseline_summary_computes_min_median_and_max() {
    ...
    let summary = summarize_baseline(
        &plan,
        vec![
            BaselineRun::new(1, plan.out_dir.join("run-001"), metrics(30, 9, 3)),
            BaselineRun::new(2, plan.out_dir.join("run-002"), metrics(10, 3, 1)),
            BaselineRun::new(3, plan.out_dir.join("run-003"), metrics(20, 6, 2)),
        ],
    )
    .expect("summary");
...
fn baseline_summary_rejects_inconsistent_row_counts() {
    let temp = TempDir::new().expect("tempdir");
    let plan = plan(temp.path(), 3);
    let mut changed_metrics = metrics(20, 6, 2);
    ...
    let error = summarize_baseline(
        ...
        vec![
            ...
            BaselineRun::new(3, plan.out_dir.join("run-003"), metrics(10, 3, 1)),
        ],
    )
...
fn metrics(scan: u128, rescan: u128, info: u128) -> DogfoodMetrics {
    let files = 20;
    let symbols = 200;
