...
- Task 5: complete (parallel-lead-commit, Lead inline review clean, lead commit b611031). Oversized transition removes rows on update+scan; update status no_change→unsupported for oversized (RELEASE-NOTE + check Miller ExtractReportLog handling at pin bump). cli.md touched outside ownership (justified, accepted).
