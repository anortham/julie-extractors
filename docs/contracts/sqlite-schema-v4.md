...
`metadata_json` is optional. `receiver_type` names the self-receiver type when
the extractor recorded one. `arity` is the call-site argument count for
languages whose function identity includes arity (Erlang `name/arity`), so a
consumer can pick `record/2` over `record/3`.
...
| `koa.route.v1` | `javascript`, `typescript` | `route_call` | parser-covered call span | `framework` | A static Koa Router verb-method route on a router built from `@koa/router` or `koa-router`; a literal constructor `prefix` becomes `route_group_prefix` and joins `effective_route_template`. |
| `hapi.route.v1` | `javascript`, `typescript` | `route_call` | route object span | `framework` | A static Hapi `server.route` object (or each object of an array) on a server built by `Hapi.server`/`new Hapi.Server`; `{param}` segments normalize to `:param`. One fact per listed method. |
...
| `drf.router_registration.v1` | `python` | `router_registration` | parser-covered call span | `framework` | A Django REST Framework `router.register(prefix, ViewSet)` on a same-file `DefaultRouter`/`SimpleRouter`. |
| `drf.viewset_action.v1` | `python` | `viewset_action` | decorated method declaration line | `framework` | A Django REST Framework `@action` extra viewset route with its verbs, `detail`, and `url_path`. |
| `drf.api_view.v1` | `python` | `api_view` | decorated function declaration line | `framework` | A Django REST Framework `@api_view` function view with its allowed verbs. |
| `spring.request_mapping.v1` | `java`, `kotlin` | `request_mapping` | class or method declaration after its modifiers (Kotlin anchors the handler `function_declaration`) | `framework` | A Spring MVC request-mapping annotation on a class or method. Java and Kotlin share this pattern id (`api_style="annotation_routing"`); both collectors are AST-driven. Java reads simple and fully qualified mapping annotations, scopes each class prefix to its own type (a nested type never resets it), joins an empty method path to the prefix without a trailing slash, and skips `@FeignClient` interfaces, which are outbound clients. The Kotlin collector is import-gated on `org.springframework.web.bind.annotation`, resets the class `@RequestMapping` prefix per `class`/`object`/`companion object`, reads Kotlin bracket-array multi-paths, and keeps `$`-interpolated / concatenated / identifier route arguments silent (M2). |
| `jaxrs.route.v1` | `java` | `route` | class or method declaration after its modifiers | `framework` | A JAX-RS / Jakarta REST route (`framework="jaxrs"`, `api_style="annotation_routing"`), import-gated on `javax.ws.rs` or `jakarta.ws.rs`. A `@Path` class emits `attribute_kind="class_route"`; a method with an HTTP method designator (`@GET`, `@POST`, …) emits `resource_method` with `verb`; a method with `@Path` and no designator emits `subresource_locator` without a verb. Method templates join the class `@Path` into `class_route_template`/`effective_route_template`. Only static string-literal `@Path` values emit. |
...
| `rails.route.v1` | `ruby` | `route` | DSL call span without its block | `framework` | A Rails routes DSL handler route, read from the AST inside a `routes.draw` block or a split `config/routes/` file. String or symbol paths; member routes join `/:id`, collection routes join the collection path, and routes directly in a `resources` block join `/:<singular>_id`. `on: :member`/`:collection`, `to:`, `"path" => "c#a"`, and `as:` are read; inside a resource an action-shaped path infers `<resource>#<path>`. An interpolated path or scope stays silent. |
| `rails.resource_route.v1` | `ruby` | `resource_route` | DSL call span without its block | `framework` | A Rails `resources` or `resource` declaration. A nested resource carries its parent path in `scope_path`; `only`/`except` read arrays, `%i[]`, and single symbols. |
| `rails.mount.v1` | `ruby` | `mount` | parser-covered DSL call span | `framework` | A Rails `mount` route for a Rack app or engine. |
| `laravel.route.v1` | `php` | `route` | parser-covered `Route` facade call span | `framework` | A static Laravel `Route` facade route (`api_style="call_routing"`). Import-gated on `Route::`; AST-driven. `Route::get/post/put/patch/delete/options` set an upper-cased `verb`; `Route::any` omits the verb; `Route::match([verbs], ...)` emits one fact per static verb. `{param}`/`{param?}` normalize to `:param`. Same-file `Route::prefix`/`group` prefixes join into `route_group_prefix`/`effective_route_template`; a non-literal prefix or path stays silent (M2). `controller_action` is captured from a `[Ctrl::class, 'm']`, `'Ctrl@m'`, or invokable `Ctrl::class` (`Ctrl@__invoke`) handler, and a bare action inside `Route::controller(Ctrl::class)->group(...)` joins the group controller. `route_name` comes from `->name('x')` plus any enclosing group `name`/`as` prefix. Metadata payload keys: see the JSON contract linked below. |
...
| `laravel.route_prefix.v1` | `php` | `route_prefix` | parser-covered `Route::prefix`/`group` prefix site | `framework` | A static same-file Laravel group prefix (`Route::prefix('x')->group(...)` or `Route::group(['prefix'=>'x'], ...)`) emitted at its own site with `mount_path` (raw literal) and `normalized_mount_path` (including enclosing group scope). A closure-less `Route::prefix('api')->group(base_path('routes/api.php'))` (or `__DIR__ . '/...'`) in a provider emits the site fact with `mount_target` naming the routes file; joining it to that file's routes is code-kb's cross-file job. |
| `symfony.route.v1` | `php` | `request_mapping` | class or method declaration carrying a static `#[Route]` attribute | `framework` | A Symfony `#[Route]` attribute route (`api_style="annotation_routing"`). Import-gated on `Symfony\Component\Routing\`. Class-level `#[Route]` emits `attribute_kind="class_route"` and joins into method `class_route_template`/`effective_route_template`. Method-level routes with `methods=` emit `attribute_kind="http_method"` (one fact per verb); without `methods=` emit `attribute_kind="request_mapping"` with verb omitted. `route_name` joins the class `#[Route(name:)]` prefix to the method `name:`. Static-literal path guarding only (M2). Distinct from `laravel.route.v1`. |
...
| `cowboy.route.v1` | `erlang` | `route` | parser-covered dispatch path-entry tuple span | `framework` | One static path entry of a `cowboy_router:compile/1` dispatch table (`api_style="dispatch_table"`). Handles `{PathMatch, Handler, Opts}` and `{PathMatch, Constraints, Handler, Opts}` entries under `{Host, Paths}` or `{Host, Constraints, Paths}` host entries. `route_template` is the string or one-segment binary path; Cowboy `:binding` segments become `dynamic_segments`; `host` is the static host match (`_` for any host) and `handler_module` the handler atom. Cowboy dispatches every method to the handler, so the fact has no `verb`. Metadata payload keys: see the JSON contract linked below. |
...
| `vue.route_reference.v1` | `vue` | `route_reference` | `template_attribute` | `frontend_navigation` | A static Vue Router link target such as `<RouterLink to="/calendar">`, or a script `router.push('/home')` / `router.replace(...)` call (`source_kind="router_navigation_call"`, no `attribute_name`). |
...
| `nuxt.route_reference.v1` | `vue` | `route_reference` | `template_attribute` | `frontend_navigation` | A static Nuxt `NuxtLink` or `nuxt-link` target with a literal `to` path, or a `navigateTo('/login')` call (`source_kind="navigate_to"`, no `attribute_name` or `component_name`). |
...
| `react.route_reference.v1` | `javascript`, `jsx`, `typescript`, `tsx` | `route_reference` | `jsx_attribute`, `call_expression` | `frontend_navigation` | A static React Router `Link` or `NavLink` target imported from React Router, or a static `navigate("/x")` call whose `navigate` is bound from React Router `useNavigate()`. |
...
| `nextjs.route_reference.v1` | `javascript`, `jsx`, `typescript`, `tsx` | `route_reference` | `jsx_attribute`, `call_expression` | `frontend_navigation` | A static `next/link` target from a string `href` or object `pathname`, or a static `router.push/replace/prefetch("/x")` call whose `router` is bound from `useRouter()` of `next/navigation` or `next/router`. |
| `angular.route_definition.v1` | `typescript` | `route_definition` | `object` | `frontend_navigation` | A static Angular Router route object with a literal `path` in a `Routes`/`Route[]` array or in the array passed to `RouterModule.forRoot/forChild` or `provideRouter`. Requires an `@angular/router` import. |
...
| `http.client_request.v1` | `javascript`, `jsx`, `typescript`, `tsx`, `vue`, `python`, `csharp`, `razor`, `go`, `java`, `kotlin`, `php`, `ruby`, `elixir`, `erlang`, `rust` | `client_request` | parser-covered call span (Java builder chains anchor the enclosing statement) | `web.http_client` | A supported outbound HTTP client call whose URL argument is a static string literal. Vue components also cover Nuxt `$fetch` (`client="ofetch"`) and `useFetch` / `useLazyFetch` (`client="nuxt"`). Java covers `java.net.http` builders (`client="java.net.http"`), Spring `RestTemplate` (`client="spring_resttemplate"`), `WebClient` (`client="spring_webclient"`) and `RestClient` (`client="spring_restclient"`) calls on a same-file declared receiver, and OpenFeign `@FeignClient` interface mapping methods (`client="openfeign"`, anchored on the method declaration). Kotlin covers Ktor (`client="ktor"`), OkHttp builders (`client="okhttp"`), Retrofit annotations (`client="retrofit"`), Spring WebClient (`client="spring_webclient"`), and RestTemplate (`client="spring_resttemplate"`), each import-gated. PHP covers Guzzle verb and `request`/`requestAsync` forms (`client="guzzle"`), Laravel `Http` (`client="laravel_http"`), Symfony HttpClient (`client="symfony_http_client"`), and cURL (`client="curl"`). Elixir covers Req (`client="req"`), Tesla (`client="tesla"`), HTTPoison (`client="httpoison"`), Finch.build (`client="finch"`), and OTP `:httpc` (`client="httpc"`). Erlang covers `httpc:request` (`client="httpc"`), `hackney:request` and the `hackney` verb functions (`client="hackney"`), and the `gun` verb functions, whose target is the request path on an open connection (`client="gun"`); string and one-segment binary URLs count. Rust covers reqwest (`client="reqwest"`), hyper builders (`client="hyper"`), and ureq (`client="ureq"`). Metadata payload keys: see the JSON contract linked below. |
...
| `razor.route_reference.v1` | `csharp`, `razor` | `route_reference` | parser-covered call/attribute-value span | `frontend_navigation` | A Razor/Blazor route reference from a NavigationManager call or an `<a>`/`<NavLink>` href. Base-relative targets resolve against `/` with `base_relative`/`raw_target`; interpolated and templated targets keep `{expression}` segments. |
| `razor.mvc_link.v1` | `razor` | `mvc_link` | tag-name span or helper call span | `server_navigation` | An MVC or Razor Pages link to a page or action from `asp-*` tag helpers, `Html.ActionLink`, `Html.BeginForm`, `Url.Action`, or `Url.Page`. |
| `razor.partial_reference.v1` | `razor` | `partial_reference` | attribute-value span or helper call span | `view_composition` | A partial view reference from `<partial name>` or an `Html.Partial*`/`RenderPartial*` call. |
| `razor.view_component_reference.v1` | `razor` | `view_component_reference` | tag-name span or call span | `view_composition` | A view component reference from `Component.InvokeAsync` or a `<vc:*>` tag helper. |
| `razor.layout_reference.v1` | `razor` | `layout_reference` | `assignment_expression`, `razor_layout_directive` | `view_composition` | A layout reference from `Layout = "..."` or `@layout`. |
| `razor.model_binding.v1` | `razor` | `model_binding` | attribute-value span | `form_binding` | A tag-helper model binding from `asp-for` or `asp-validation-for`. |
...
`fetch()`, Nuxt `$fetch` / `useFetch` / `useLazyFetch`, and axios calls emit `http.client_request.v1` only when the first
...
nothing rather than silently degrading to `GET`. `fetch` and the Nuxt helpers are globals, so no
...
bare verb macros emit `phoenix.route.v1`, `live` emits a GET
`phoenix.route.v1` with `handler_kind="live_view"`, `resources` emits
`phoenix.resource_route.v1` (a `resources` do-block nests its routes under the
member path), and `forward` emits a `phoenix.forward.v1` prefix
registration; same-file `scope` blocks are lexical-containment prefixes
(joined/poisoned like the Rails `scope_stack`) whose aliases qualify
`controller_module`, and `pipe_through`/`socket`/`channel` plus cross-file
scope prefixes are out of scope. axum routes
