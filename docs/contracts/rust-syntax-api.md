...
|---|---|---|---|
| `UnsupportedLanguage { path }` | Extension or file name unrecognized among all 42 supported languages | No | Returns unmutated caller path |
...
- **Extensionless Files:** Exact base names (e.g. `qmldir`, `go.mod` for `gomod`, and `go.sum` for `gosum`) are matched case-insensitively. An R package `NAMESPACE` file (exact, upper-case base name) selects `r`. Shell startup files (`.bashrc`, `.bash_profile`, `.bash_login`, `.bash_logout`, `.bash_aliases`, `.profile`, `.envrc`) and files whose first line is a `sh`, `bash`, or `bats` shebang select `bash`.
...
## 8. Capability Fixture Ledger (42 Languages)
...
| `gosum` | `fixtures/extraction/gosum/basic/go.sum` |
