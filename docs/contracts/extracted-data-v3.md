...
| `pending_relationships` | Deferred relationship targets with terminal name, receiver, namespace, import context, and caller scope. For a qualified call chain, the receiver is the last qualifier before the terminal name and the namespace path holds the qualifiers before it. Rust, Elixir, F#, and Erlang record a path-style call (`a::b::f()`, `A.B.f()`, `m:f()`) with every qualifier in the namespace path and no receiver. | Languages with `pending_relationships: true` when resolution needs another file or pass. |
