...
| code-kb | deterministic graph candidates over those extracted facts |
...
`kind_coverage.test_detection`. code-kb may use those facts to compute
...
evidence remain unknown. code-kb must not convert unknown extraction evidence
