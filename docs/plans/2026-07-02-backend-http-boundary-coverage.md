...
- Naming rule: client-side references carry `target_path` + `url_kind`; handler definitions carry `route_template` (raw, exactly as written) and `normalized_route_template` (the cross-family join key — see Cross-Family Doctrine). Do not rename existing ASP.NET keys.
...
1. **`normalized_route_template` is the universal server-side join key.** Every handler fact emits it: compute `normalize(effective_route_template if present else route_template)` where `normalize` (a) ensures a leading `/`, (b) converts each framework's param syntax to the `:param` flavor (`{id}`→`:id`, `<int:id>`→`:id`, `{id:int}`→`:id`, `*filepath`→`:filepath`, `{path...}`→`:path`), and (c) strips converter/constraint annotations. The normalizer lives in `base/http_boundary.rs` with table-driven per-framework flavor rules and exhaustive unit tests. Regex route syntaxes (Django `re_path`) cannot be honestly normalized — those facts omit `normalized_route_template` and set `route_syntax="regex"`.
...
- [ ] `http_boundary` normalizer handles every flavor in the doctrine table with unit tests per flavor, including catch-all and constraint-annotation cases.
...
- [ ] `app.route("/x").get(h).post(h)` emits two facts; `app.all` omits verb; `app.use("/api", usersRouter)` emits a mount fact with `mount_target=usersRouter`.
...
- [ ] `path("users/<int:pk>/", views.detail, name="user-detail")` emits `route_syntax=path`, `normalized_route_template=/users/:pk`, `route_name=user-detail`, no verb; `re_path` emits `route_syntax=regex` with no normalized template; `include()` emits mount facts.
...
**Implementation tier:** Tasks 1–8 implementation once contracts are locked (narrow file ownership, explicit ceilings, no parser-dependency changes). Harness mapping: inherit.
