...
**Admission measurement and pruned-log amendment (2026-09-05):** Instrument the actual executed admission path, not a replay of its SQL. The pinned rusqlite API exposes VM-step and full-scan-step counters but not SQLite scanstatus row-visit APIs; report these supported counters honestly alongside statement counts and query plans. Compare per-statement VM work across adversarial 1k/10k/100k fixtures to catch indexed range scans that a full-scan counter alone misses. Measure first-time catalog/floor activation separately from first admission. A healthy current manifest may outlive its receipted `manifest_flipped` log row: capture `served_store_log_sequence` from the maximum retained committed log sequence, or zero for an empty log. Use the original flip sequence as the retention floor when still present; otherwise use the served sequence conservatively. Reserved allocator high-water is not a committed log position. Missing pruned history does not require a rebuild or prevent admission. Capture/re-probe owner identity inside the admission transaction. Ordinary writer leases may exclude new admission, but do not block coordinator-only renew/release; foreign maintenance intent still blocks all three.

**Integration prerequisite discovered in review:** Existing-family opens do not run coordinator schema creation. Task1's bounded activation therefore also owns legacy catalog installation: under its existing maintenance fence, install the wholly absent reader catalog atomically before the permanent floor reaches reader capability. Use one authoritative DDL for fresh and legacy initialization. Missing/partial/malformed catalog after reader enablement is corruption and must refuse, never recreate empty registration state. Distinguish the permanent maintenance source floor from a temporary raised store floor. A crate-private read-only catalog validator is shared with Task4. This follow-up is tracked as Task1 legacy-catalog correction; it is required before the CLI handoff, not deferred work.
...
**Approach:** Follow `advance_consumer_cursor`'s validation and transaction pattern, but bind the registration to an immutable snapshot fingerprint. Re-read the current view/manifest rows inside the transaction; reject a changed generation as `stale_snapshot`. Renew updates only heartbeat/expiry after matching pin ID, nonce, PID, and producer-captured birth identity. Release deletes the single registration atomically; there are no child/version-root rows. An absent row is an idempotent no-op regardless of nonce, while an existing row requires the exact nonce. Never return the nonce to a caller that did not present it.
...
**Review amendment (2026-09-05):** Absence is meaningful only in the original identity domain. A readable Linux boot ID does not make `/proc/<pid>` ENOENT definitive: `hidepid` can hide a living process, and another PID namespace can omit it. Bind captured Linux identity to boot and PID-namespace identity; validate that domain before using absence and require an independent kernel existence result rather than proc ENOENT alone. Unknown/denied existence checks retain. Likewise, a birth identity from another platform or host cannot be qualified as dead by a local absent PID. The probe seam must default to refusal when it cannot validate the original domain. Windows must distinguish an exited process object whose handle remains open from a live process using documented wait/status behavior, not creation time alone or undefined exit-time contents. Add deterministic hidden-live-process/domain-mismatch tests plus a real Windows exited-child test with the parent handle still held. These checks strengthen death qualification without adding a wire field: the birth string remains producer-internal and opaque to Miller.

**Platform implementation decision:** Preserve the workspace's `unsafe_code = "forbid"`. Use safe `rustix` process probes on Linux and minimal-feature `winsafe` process wrappers on Windows. Windows domain capture uses the public `machine-uid` library's documented OS-install identity, validated as a nonzero GUID and SHA-256 domain-hashed before storing; never log the raw machine ID. Missing/malformed/domain-changed identities fail closed. These target-specific dependencies are normal implementation scope, not a release or model download. `GetCurrentHwProfile` was considered but the selected safe wrapper does not expose it; do not introduce a new unsafe wrapper crate or weaken Windows cleanup to permanently unverified merely to avoid implementing the required platform proof.
