...
- **Full resolution pass: 1,212 ms** at ~92k identifiers — within the < 2 s
  design budget (~1.6× headroom).
- **Single-file delta pass: ~110 ms.**

The pass is set-based SQL over the writer transaction; two identical scans
produce byte-identical resolution tables.

## Known Limitations

Both are measured and enforced by the performance gate, not hidden:

- **Single-file delta marginally exceeds the 100 ms target (~110 ms) at
  92k-identifier scale.** This is not delta-size-driven: each pass rebuilds the
  whole-workspace candidate index, identifier locator, and covered-set once,
  which is an O(workspace) fixed cost that already exceeds 100 ms before any
  delta-specific work. Scoping those loads to the changed files is a tracked
  optimization opportunity; the cost grows with workspace size.
- **A delta touching ≥ 16,384 distinct symbol names hits SQLite's compiled
  bound-variable limit** (the by-names worklists bind `2 × N` variables against a
  32,766 ceiling). The behavior is graceful: the resolver returns a non-fatal
  error, the scan still commits its extraction rows, and that delta's resolution
  degrades to `failed` (recorded in `reference_resolution_status`) until the
  next full pass. No panic, no data loss. Chunking the `IN (...)` binds is the
  tracked closure; ordinary single-file deltas are far below the limit.
