...
- Trigger: tag push (`v2.9.0`)
- Result: success
- Published: `2026-07-07T13:04:34Z`

Local prep gates run before publish (all green at the release commit):
...
Additional prep gates at the release commit: `cargo test -p xtask`,
`cargo xtask test default`, `cargo xtask test contract`, and
`cargo xtask release preflight --version 2.9.0` (ok, 4 targets, 19 inputs).
The perf gate ran on the release profile: full pass 1,282 ms (< 2,000 ms
budget), single-file delta 84 ms (< 100 ms target).

Published-asset verification after release:

- Remote tag `v2.9.0` points at
  `eb3eaee118592505da4d6bba96166c784a2ce198`.
- The GitHub release is not a draft and not a prerelease.
- The downloaded Apple Silicon asset's SHA-256 digest matches GitHub release
  metadata, and the packaged checksum file verifies the extracted
  `dist/aarch64-apple-darwin/julie-extract` binary.
- The downloaded Apple Silicon binary reports `julie-extract 2.9.0`.
- Downloaded-binary dogfood evidence is recorded in
